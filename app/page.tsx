...
function compute(data: Record<string, unknown> | null) {
  ...
  // Net worth history（現金餘額趨勢）: 1 point per month, last 6 months
  ...
  // ── #4 Rolling 12 個月 ETF 實收股息（憲法 4.3）──
  const now = new Date();
  ...
  // 本月買入金額（Trade Log Buy，r[12] = Total Amount TWD）
  ...
  return {
    ...
    monthBuyAmt,           // #1
    rolling12Div, etfDivGap, // #4
    benchmarkName, benchmarkAnnRet, // #3
    realizedAvgAnnReturn, realizedTotalPnL, // #3
    isStress, // #5
  };
}
...
// #2 LineChart — 標題改為「現金餘額趨勢」，移到再平衡 Tab
  ...
  return (
    <div>
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H + 14}`} style={{ display: 'block', overflow: 'visible' }}>
        ...
        <path d={linePath} fill="none" stroke="#2563eb" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" opacity={0.6} />
        {pts.map((p, i) => (
          <g key={i}>
            <circle cx={p.x} cy={p.y} r={i === pts.length - 1 ? 4 : 2.5} fill="#2563eb" opacity={i === pts.length - 1 ? 0.8 : 0.35} />
    ...
    <div style={{ minHeight: '100vh', background: '#f7f6f3', fontFamily: "'DM Sans', system-ui", color: '#1a1a1a' }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .tab-btn{font-size:13px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;padding:14px 16px;border:none;background:none;cursor:pointer;color:#666;border-bottom:2.5px solid transparent;transition:all .2s;white-space:nowrap;font-family:'DM Sans',system-ui}
        ...
        .wc{background:#fefce8;color:#713f12;border:1px solid #fde68a;border-radius:4px;padding:9px 14px;font-size:13px;font-weight:500;line-height:1.5}
        .wc-stress{background:#fef2f2;color:#991b1b;border:1px solid #fecaca;border-radius:4px;padding:12px 14px;font-size:13px;font-weight:600;line-height:1.6}
        ...
        @media(max-width:680px){
          ...
          .wc{font-size:15px!important;padding:12px 14px!important}
          ...
          .nw-label{display:none!important}
          .nw-meta{display:none!important}
          ...
          .bottom-nav button.active{color:#1a1a1a;border-top-color:#1a1a1a}
      ...
      {pullY > 0 && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', height: pullY, background: 'transparent', pointerEvents: 'none' }}>
          <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#fff', border: '1.5px solid #e6e2dc', boxShadow: '0 2px 8px rgba(0,0,0,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, transform: `rotate(${(pullY / PULL_THRESHOLD) * 180}deg)`, transition: 'transform .05s', opacity: Math.min(pullY / PULL_THRESHOLD, 1) }}>↻</div>
        </div>
      )}

      <div style={{ background: '#fff', borderBottom: '1px solid #e6e2dc', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '13px 0 0', gap: 10 }}>
            <div>
              <div style={{ fontSize: 10, letterSpacing: '.14em', textTransform: 'uppercase', color: '#999', marginBottom: 2 }}>Investment OS · Live</div>
              <div className="serif" style={{ fontSize: 18, fontWeight: 700, lineHeight: 1.2 }}>Portfolio Dashboard</div>
            </div>
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              <div className="nw-label" style={{ fontSize: 10, color: '#999', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 2 }}>Total Net Worth</div>
              <div className="serif" style={{ fontSize: 20, fontWeight: 700 }}>{fmtTWD(c.netWorth)}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'flex-end', marginTop: 3 }}>
                <span className="nw-meta" style={{ fontSize: 10, color: '#999' }}>USD/TWD {c.fx.toFixed(2)} · {lastUpdated?.toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit' })}</span>
                <button onClick={refetch} style={{ background: 'none', border: '1.5px solid #e6e2dc', borderRadius: 4, padding: '4px 10px', fontSize: 13, fontWeight: 600, color: '#555', cursor: 'pointer' }}>↻</button>
              </div>
            </div>
          </div>
          <div className="top-tabs" style={{ display: 'flex', overflowX: 'auto', marginTop: 2, WebkitOverflowScrolling: 'touch' }}>
            {TABS.map(([k, v]) => (
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '16px 16px 90px' }}>

        {/* Warnings — #5 Stress 優先顯示，樣式不同 */}
        {c.warns.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14 }}>
            {c.warns.map((w, i) => (
              <div key={i} className={i === 0 && c.isStress ? 'wc-stress' : 'wc'}>
                {w}
              </div>
            ))}
          </div>
        ...
        {tab === 'overview' && (
          <div className="fu" style={{ display: 'grid', gap: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                  ...
                  {c.growthS && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                      <span style={{ fontSize: 12, color: '#777' }}>成長</span>
                      <span style={{ fontSize: 15, fontWeight: 700, color: c.growthS === '↑' ? '#16a34a' : '#dc2626' }}>{c.growthS}</span>
                    </div>
                  )}
                  {c.inflS && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                      <span style={{ fontSize: 12, color: '#777' }}>通膨</span>
                      <span style={{ fontSize: 15, fontWeight: 700, color: c.inflS === '↑' ? '#dc2626' : '#16a34a' }}>{c.inflS}</span>
                    </div>
                  )}
                  <div style={{ fontSize: 15, fontWeight: 700 }}>{c.quadrant}</div>
                  {c.quadConfirm && c.quadConfirm.includes('確認') && (
                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 3, fontWeight: 600, background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0' }}>{c.quadConfirm}</span>
                  )}
                </div>
                {c.fxTrend && (
            ...
            <div className="g5" style={{ display: 'grid', gap: 10 }}>
              {[
                ...
                <div key={i} className="card card-hover" style={{ padding: '14px 16px' }}>
                  ...
                  <div className="serif" style={{ fontSize: 17, fontWeight: 700, color: (k as {green?:boolean;red?:boolean}).green ? '#16a34a' : (k as {green?:boolean;red?:boolean}).red ? '#dc2626' : '#1a1a1a', lineHeight: 1.3 }}>{k.val}</div>
            ...
            <div className="card card-hover" style={{ padding: '14px 18px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 9, alignItems: 'flex-end' }}>
                <div>
                  ...
                  <div style={{ fontSize: 14, color: '#555' }}>{fmtTWD(c.netWorth)} <span style={{ color: '#bbb' }}>/ {fmtTWD(c.targetNW)}</span></div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div className="serif" style={{ fontSize: 22, fontWeight: 700 }}>{fmtPct(c.goalPct)}</div>
                  <div style={{ fontSize: 12, color: '#999' }}>{c.estMonths ? `預估 ${c.estMonths} 個月達成` : '計算中'}</div>
                </div>
              </div>
            ...
            {/* #2 Overview 不再有 LineChart，空間留給未來 B3 快照 */}
            ...
            <div className="g2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 14 }}>
                    {c.awRows.map((r, i) => (
                      <div key={i}>
                        ...
                        <div style={{ fontSize: 11, color: '#777', marginTop: 3 }}>{r.gap > 0 ? `缺口 ${fmtTWD(r.gapAmt)}` : `超配 ${fmtPct(Math.abs(r.gap))}`}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div style={{ display: 'grid', gap: 12 }}>
                  ...
                  {[
                    ...
                    <div key={i} style={{ marginBottom: i === 0 ? 12 : 0 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                        ...
                        <span style={{ fontSize: 13, color: '#555' }}>{fmtPct(r.pct)}</span>
                      </div>
                      <div className="bar">
                        <div className="bf" style={{ width: mounted ? `${r.pct * 100}%` : '0%', background: r.color, opacity: .65 }} />
                      </div>
                      <div style={{ fontSize: 12, color: '#777', marginTop: 3 }}>{fmtTWD(r.val)}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="card card-hover">
                  ...
                  <div style={{ fontSize: 12, color: '#777', marginBottom: 3 }}>月被動收益</div>
                  ...
                  {c.rolling12Div > 0 && (
                    <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid #f0ede8' }}>
                      <div style={{ fontSize: 11, color: '#999', marginBottom: 4 }}>ETF 實收（近 12 月）</div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div className="serif" style={{ fontSize: 13, fontWeight: 700 }}>{fmtTWD(c.rolling12Div)}</div>
                        <div style={{
                          fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 3,
                          background: Math.abs(c.etfDivGap) > 0.1 ? '#fef2f2' : '#f0fdf4',
                          color: Math.abs(c.etfDivGap) > 0.1 ? '#dc2626' : '#16a34a',
                          border: `1px solid ${Math.abs(c.etfDivGap) > 0.1 ? '#fecaca' : '#bbf7d0'}`,
                        }}>
                          vs 估算 {c.etfDivGap >= 0 ? '+' : ''}{fmtPct(c.etfDivGap)}
                          {Math.abs(c.etfDivGap) > 0.1 ? ' ⚠ 建議校正' : ' ✓'}
                        </div>
                      </div>
                    </div>
                  )}
              ...
              <div className="card card-hover">
                ...
                <div className="g4" style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 12 }}>
                  {[
                    ...
                    { lbl: '本月買入', v: c.monthBuyAmt },
                  ].map((r, i) => (
                    <div key={i}>
                      <div style={{ fontSize: 12, color: '#777', marginBottom: 4 }}>{r.lbl}</div>
                      <div className="serif" style={{
                        fontSize: 15, fontWeight: 700,
                        color: i === 4 ? (r.v > 0 ? '#d97706' : '#ccc') : (r.v > 0 ? '#1a1a1a' : '#ccc')
                      }}>
                        {r.v > 0 ? (i === 4 ? `−${fmtTWD(r.v)}` : fmtTWD(r.v)) : '—'}
                      </div>
                    </div>
                  ))}
                </div>
                <div style={{ fontSize: 11, color: '#bbb', marginTop: 8 }}>
                  ＊本月買入為現金流出，市值變動需月末快照功能才能計算
                </div>
              </div>
            )}
        ...
        {tab === 'positions' && (
          <div className="fu card card-hover">
            ...
            <div className="ptable-header" style={{ display: 'grid', gridTemplateColumns: '80px 1fr 100px 84px 60px', gap: 8, paddingBottom: 9, borderBottom: '2px solid #f0ede8' }}>
            ...
            {c.positions.map((p, i) => (
              <div key={i} className="ptable row-hover" style={{ display: 'grid', gridTemplateColumns: '80px 1fr 100px 84px 60px', gap: 8, alignItems: 'center', padding: '13px 4px', borderBottom: '1px solid #f0ede8' }}>
                ...
                <div className="pos-name">
                  ...
                  <div className="pos-name-sub" style={{ fontSize: 11, color: '#888', marginTop: 2 }}>
                ...
                <div className="pos-detail" style={{ fontSize: 12, color: '#888', marginTop: 1 }}>
                  {p.sh.toLocaleString()} 股 · 均 {fmtNum(p.avgCost)} {p.currency}
                  {p.cp > 0 && <span> · 現 {fmtNum(p.cp)}</span>}
                </div>
                <div className="pos-mv serif" style={{ fontSize: 14, fontWeight: 700 }}>{fmtTWD(p.mvTWD)}</div>
                <div className="pos-pnl serif" style={{ fontSize: 14, fontWeight: 700, color: p.pnlTWD >= 0 ? '#16a34a' : '#dc2626' }}>
                  {p.pnlTWD >= 0 ? '+' : ''}{fmtTWD(p.pnlTWD)}
                </div>
                <div className="pos-ret" style={{ fontSize: 14, color: p.ret >= 0 ? '#16a34a' : '#dc2626', fontWeight: 700 }}>
                  {p.ret >= 0 ? '+' : ''}{fmtPct(p.ret)}
                </div>
                <div style={{ gridColumn: '1 / -1', paddingTop: 6 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                    <span style={{ fontSize: 10, color: '#bbb' }}>配置比重</span>
                    <span style={{ fontSize: 10, color: '#bbb' }}>{fmtPct(p.portPct)}</span>
                  </div>
          ...
          <div className="fu" style={{ display: 'grid', gap: 12 }}>
            <div className="card card-hover">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                ...
                <span style={{ fontSize: 12, color: '#999' }}>固定執行，不受象限影響</span>
              </div>
              {c.dcaPlan.map((d, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '13px 0', borderBottom: i < c.dcaPlan.length - 1 ? '1px solid #f0ede8' : 'none' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: 12, color: '#bbb', marginTop: 2 }}>殘差 {fmtTWD(d.residual)}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="card card-hover">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 12, color: '#999' }}>彈性執行，依缺口補足</span>
              </div>
              <div style={{ fontSize: 13, color: '#777', marginBottom: 14 }}>可動用現金 {fmtTWD(c.avail)} · 象限：{c.quadrant}</div>
              {c.awRows.map((r, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '13px 0', borderBottom: i < c.awRows.length - 1 ? '1px solid #f0ede8' : 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 9, height: 9, borderRadius: '50%', background: CAT_COLOR[r.cat] }} />
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 600 }}>{r.cat}</div>
                      <div style={{ fontSize: 12, color: '#777', marginTop: 2 }}>現況 {fmtPct(r.cur)} → 目標 {fmtPct(r.tgt)}</div>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <span className={`bdg ${r.gap > 0.01 ? 'buy' : r.gap < -0.01 ? 'over' : 'ok'}`}>
                    ...
                    <div className="serif" style={{ fontSize: 15, fontWeight: 700, marginTop: 4, color: r.gap > 0 ? '#16a34a' : '#dc2626' }}>
            ...
            <div className="card card-hover">
              ...
              {(() => {
                ...
                return suggestions.map(({ p, sh, amt }, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '13px 0', borderBottom: '1px solid #f0ede8' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div className="serif" style={{ fontSize: 15, fontWeight: 700, color: '#16a34a' }}>{fmtTWD(amt)}</div>
                  </div>
                ));
              })()}
            </div>
          ...
          <div className="fu" style={{ display: 'grid', gap: 12 }}>
            <div className="card card-hover">
              ...
              <div style={{ background: '#faf9f7', border: '1px solid #e6e2dc', borderRadius: 4, padding: '10px 14px', marginBottom: 16 }}>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.09em', textTransform: 'uppercase', color: '#888', marginBottom: 6 }}>All Weather 缺口參考</div>
                  ...
                  <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
                    {(['list', 'custom'] as const).map(m => (
                      <button key={m} onClick={() => setSimRows(r => r.map((x, j) => j === i ? { ...x, mode: m } : x))}
                        style={{ fontSize: 12, fontWeight: 600, padding: '5px 12px', borderRadius: 3, cursor: 'pointer', border: '1.5px solid', borderColor: row.mode === m ? '#1a1a1a' : '#e6e2dc', background: row.mode === m ? '#1a1a1a' : 'transparent', color: row.mode === m ? '#fff' : '#888', fontFamily: "'DM Sans',system-ui" }}>
                        {m === 'list' ? '從清單選' : '手動輸入'}
                      </button>
                    ))}
                  </div>
              ...
              <div className="card card-hover">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <div className="serif" style={{ fontSize: 19, fontWeight: 700 }}>{fmtTWD(simTotal)}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 12, color: '#777', marginBottom: 3 }}>執行後可動用現金</div>
                    <div className="serif" style={{ fontSize: 19, fontWeight: 700, color: c.avail - simTotal < 0 ? '#dc2626' : '#1a1a1a' }}>{fmtTWD(c.avail - simTotal)}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="card card-hover">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
                <div>
                  ...
                  <div className="serif" style={{ fontSize: 18, fontWeight: 700, color: c.realizedTotalPnL >= 0 ? '#16a34a' : '#dc2626' }}>
                    {c.realizedTotalPnL >= 0 ? '+' : ''}{fmtTWD(c.realizedTotalPnL)}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: 12, color: '#777', marginBottom: 4 }}>已實現平均年化</div>
                  <div className="serif" style={{ fontSize: 18, fontWeight: 700, color: c.realizedAvgAnnReturn && c.realizedAvgAnnReturn >= 0 ? '#16a34a' : '#dc2626' }}>
                    {c.realizedAvgAnnReturn !== null
                      ? `${c.realizedAvgAnnReturn >= 0 ? '+' : ''}${fmtPct(c.realizedAvgAnnReturn)}`
                      : '—'}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: 12, color: '#777', marginBottom: 4 }}>基準（{c.benchmarkName}）年報酬</div>
                  <div className="serif" style={{ fontSize: 18, fontWeight: 700, color: '#555' }}>
                    {c.benchmarkAnnRet > 0
                      ? `${fmtPct(c.benchmarkAnnRet)}`
                      : <span style={{ fontSize: 13, color: '#bbb' }}>未填入（Settings 區塊三）</span>}
                  </div>
                </div>
              </div>
              {c.benchmarkAnnRet > 0 && c.realizedAvgAnnReturn !== null && (
                <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid #f0ede8' }}>
                  <div style={{ fontSize: 12, color: '#777', marginBottom: 4 }}>超額報酬（已實現年化 − 基準）</div>
                  <div className="serif" style={{ fontSize: 20, fontWeight: 700, color: (c.realizedAvgAnnReturn - c.benchmarkAnnRet) >= 0 ? '#16a34a' : '#dc2626' }}>
                    {(c.realizedAvgAnnReturn - c.benchmarkAnnRet) >= 0 ? '+' : ''}
                    {fmtPct(c.realizedAvgAnnReturn - c.benchmarkAnnRet)}
                  </div>
              ...
              <div style={{ fontSize: 11, color: '#bbb', marginTop: 10 }}>
                ＊基準年報酬率請每年底手動填入 Settings 區塊三「機會成本基準年報酬率」
              </div>
            </div>

            {/* 個別已實現紀錄 */}
            <div className="card card-hover">
              <div className="lbl">已實現明細</div>
                  ...
                  <div key={i} className="row-hover" style={{ padding: '16px 4px', borderBottom: i < c.realizedRows.length - 1 ? '1px solid #f0ede8' : 'none' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <div style={{ fontSize: 13, color: '#888', marginTop: 2 }}>報酬率 {fmtPct(r.returnPct)}</div>
                      </div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
                      {[
                        ...
                        <div key={j}>
                          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: '#888', marginBottom: 4 }}>{cc.lbl}</div>
                          <div style={{ fontSize: 14, fontWeight: 600 }}>{cc.v}</div>
                        </div>
                      ))}
                    </div>
